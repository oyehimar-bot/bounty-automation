...
watcher/    Poll GitHub + Drips for open bounty issues, draft applications
            (via Claude, Gemini, or OpenCode - see providers.js), and review
            or post them with apply.js
...
- **A drafting CLI**: Claude Code (default, uses your existing subscription;
  no API key), Gemini CLI, or OpenCode — see
...
```json
"draft_provider": "claude",   // or "gemini" or "opencode"
"draft_model": "haiku",
```
...
Override the binary path with `CLAUDE_BIN`, `GEMINI_BIN`, `OPENCODE_BIN`, or
generically `DRAFT_BIN`, if the CLI isn't on `PATH` for the process running
the watcher. Add `--verbose` to `node watch.js` / `node drips.js` to see the
underlying CLI's stderr when a draft fails, instead of just the one-line
summary.
...
│   ├── providers.js                          drafting backend: claude / gemini / opencode
...
│   ├── applied.json, state.json, apply-state.json   local state (gitignored)
...
- **`draft failed` / drafts empty** — run `node watch.js --verbose` (or
  ...
  - Any provider: the CLI isn't on `PATH` for the process running the
    ...
    `OPENCODE_BIN` / `DRAFT_BIN` to its full path, or confirm it works when
    you run it by hand first, e.g. `echo hi | claude -p`.
  - Gemini/OpenCode: not logged in, or (OpenCode) no default model
    configured — run `gemini` or `opencode` interactively once to finish
    login/setup before using them headlessly here.
